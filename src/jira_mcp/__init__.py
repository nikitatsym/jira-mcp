...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="jira-mcp",
        description="MCP server for Jira Cloud. Serves MCP over stdio unless --http is given.",
    )
    parser.add_argument(
        "--http",
        action="store_true",
    ...
    # Refuse to start on a credential the backend rejects, instead of failing the first call.
    _get_client().check()
